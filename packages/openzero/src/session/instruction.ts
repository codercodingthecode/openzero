...
function globalFiles() {
  ...
  if (Flag.OPENZERO_CONFIG_DIR) {
    files.push(path.join(Flag.OPENZERO_CONFIG_DIR, "AGENTS.md"))
  }
  files.push(path.join(Global.Path.config, "AGENTS.md"))
  if (!Flag.OPENZERO_DISABLE_CLAUDE_CODE_PROMPT) {
...
async function resolveRelative(instruction: string): Promise<string[]> {
  ...
  if (!Flag.OPENZERO_CONFIG_DIR) {
    log.warn(
      `Skipping relative instruction "${instruction}" - no OPENZERO_CONFIG_DIR set while project config is disabled`,
    )
    return []
  }
  return Filesystem.globUp(instruction, Flag.OPENZERO_CONFIG_DIR, Flag.OPENZERO_CONFIG_DIR).catch(() => [])
}
