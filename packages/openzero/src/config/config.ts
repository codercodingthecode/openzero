...
export namespace Config {
  ...
  const managedConfigDir = process.env.OPENZERO_TEST_MANAGED_CONFIG_DIR || getManagedConfigDir()
  ...
  export const state = Instance.state(async () => {
    ...
    if (Flag.OPENZERO_CONFIG) {
      result = merge(result, await loadFile(Flag.OPENZERO_CONFIG))
      log.debug("loaded custom config", { path: Flag.OPENZERO_CONFIG })
    }

    // Project config overrides global and remote config.
    if (!Flag.OPENZERO_DISABLE_PROJECT_CONFIG) {
    ...
    const directories = [
      ...
      ...(!Flag.OPENZERO_DISABLE_PROJECT_CONFIG
    ...
    if (Flag.OPENZERO_CONFIG_DIR) {
      directories.push(Flag.OPENZERO_CONFIG_DIR)
      log.debug("loading config from OPENZERO_CONFIG_DIR", { path: Flag.OPENZERO_CONFIG_DIR })
    }

    const deps = []

    for (const dir of unique(directories)) {
      if (dir.endsWith(".opencode") || dir === Flag.OPENZERO_CONFIG_DIR) {
        for (const file of ["opencode.jsonc", "opencode.json"]) {
    ...
    if (process.env.OPENZERO_CONFIG_CONTENT) {
      result = merge(
        result,
        await load(process.env.OPENZERO_CONFIG_CONTENT, {
          dir: Instance.directory,
          source: "OPENZERO_CONFIG_CONTENT",
        }),
      )
      log.debug("loaded custom config from OPENZERO_CONFIG_CONTENT")
    }
    ...
    if (Flag.OPENZERO_PERMISSION) {
      result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.OPENZERO_PERMISSION))
    }
