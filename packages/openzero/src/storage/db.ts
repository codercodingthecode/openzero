...
declare const OPENZERO_MIGRATIONS: { sql: string; timestamp: number }[] | undefined
...
export namespace Database {
  ...
  export const Client = lazy(() => {
    ...
    const entries =
      typeof OPENZERO_MIGRATIONS !== "undefined"
        ? OPENZERO_MIGRATIONS
        : migrations(path.join(import.meta.dirname, "../../migration"))
    if (entries.length > 0) {
      log.info("applying migrations", {
        count: entries.length,
        mode: typeof OPENZERO_MIGRATIONS !== "undefined" ? "bundled" : "dev",
      })
      migrate(db, entries)
    }
